...
const sideQuestRecords = [
  {
    ...
    notes:
      "Side quest dari Lefina dengan beberapa objective sebelum reward diterima.",
...
export default function EditSideQuestPage() {
  const navigate =
    useNavigate()
  ...
  const [
    ...
    repeatable:
      existingQuest.repeatable
        ? "YES"
        : "NO",

    expReward:
      existingQuest.rewards
        ?.exp != null
        ? String(
            existingQuest
              .rewards.exp,
  ...
  function handleSave(
    ...
    /*
      ...
      await updateSideQuest(
        existingQuest.id,
        payload,
      )

      navigate(
        `/data/worlds/side-quests/${existingQuest.id}`,
      )
    */
      ...
      <header className="mt-5 flex items-end justify-between gap-8">
        <div>
          ...
          <p className="mt-2 max-w-[780px] text-sm leading-6 text-muted-foreground">
            Perbarui informasi side quest, objective, reward,
            dan NPC terkait.
          </p>
        </div>
