...
const emblemRecords = [
...

  {
    ...
    description:
      "Emblem mingguan yang memberikan hadiah item setelah memenuhi aktivitas tertentu.",
    ...
    notes:
      "Reward item dapat berubah apabila data resmi diperbarui.",
...
export default function EditEmblemPage() {
  ...
  const [
    ...
    description:
      existingEmblem.description ||
      "",

    category:
      existingEmblem.category ||
      "",

    frequency:
      existingEmblem.frequency ||
      "",

    condition:
      createConditionForm(
        existingEmblem.condition,
      ),

    reward:
      createRewardForm(
        existingEmblem.reward,
      ),
  ...
  function handleSave(
    ...
    /*
      ...
      navigate(
        `/data/character-system/emblems/${existingEmblem.id}`,
      )
    */
      ...
      <header className="mt-5 flex items-end justify-between gap-8">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-[760px] text-sm leading-6 text-muted-foreground">
            Perbarui informasi emblem, kondisi pencapaian, dan
            reward yang diterima pemain.
          </p>
        </div>

        <StatusBadge
          status={
            existingEmblem.status
          }
        />
      </header>
        ...
        <div className="flex items-center justify-between gap-8 border-t border-border bg-muted/20 px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              Simpan perubahan sebagai Draft atau publish untuk memperbarui data.
            </p>
          </div>
  ...
  return (
    <Field>
      ...
      <div className="flex h-11 cursor-not-allowed items-center gap-3 rounded-lg border border-border/90 bg-muted/85 px-3.5 shadow-inner">
        ...
        <span className="min-w-0 flex-1 truncate text-sm font-medium text-muted-foreground/75">
          {value}
        </span>
      </div>
